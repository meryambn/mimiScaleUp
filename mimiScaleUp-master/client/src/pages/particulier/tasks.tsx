...
const ParticulierTasksPage = () => {
  ...
  const handlePhaseChange = (phase: number | string | null): void => {
            ...
            <main className="main-content">
              ...
              <Tabs defaultValue="all" onValueChange={setActiveTab}>
                ...
                <TabsContent value="all">
                  {filteredTasks.length === 0 ? (
                    ...
                    <div>
                      {/* Tasks by Status */}
                      {(['todo', 'in_progress', 'completed'] as const).map(status => {
                        const statusTasks = filteredTasks.filter((task: Task) => task.status === status ||
                ...
                <TabsContent value="overdue">
                  {filteredTasks.filter((task: Task) => !task.completed && task.dueDate && new Date(task.dueDate) < new Date()).length === 0 ? (
                    <div className="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {filteredTasks
                        .filter((task: Task) => !task.completed && task.dueDate && new Date(task.dueDate) < new Date())
                        .map((task: Task) => (
                          <motion.div
                            key={task.id}
                            className="task-card"
