...
const StartupHeader: React.FC<StartupHeaderProps> = ({ onToggleSidebar }) => {
  ...
  const navLinks = [
    { name: 'Tableau de bord', href: '/startup/dashboard' },
    { name: 'Profil', href: '/startup/profile' }
  ];
  ...
  return (
    <header className="admin-header relative z-10 flex-shrink-0 flex h-16 bg-white border-b border-gray-200">
      <button
        type="button"
      ...
      <div className="flex-1 px-4 flex justify-between">
        ...
        <div className="admin-header-actions flex items-center space-x-1 ml-2">
          <Button variant="ghost" size="sm" className="rounded-full h-6 w-6 p-0">
            <HelpCircle className="h-3 w-3 text-gray-500" />
          </Button>
          <Button variant="ghost" size="sm" className="rounded-full h-6 w-6 p-0 relative" onClick={() => setMessagesOpen(true)}>
            <MessageSquare className="h-3 w-3 text-gray-500" />
            {unreadMessageCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 flex h-2 w-2 items-center justify-center rounded-full bg-primary text-[6px] font-bold text-white">{unreadMessageCount}</span>
            )}
          </Button>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuItem className="cursor-pointer" onClick={() => window.location.href = '/startup/profile'}>
                Profil
              </DropdownMenuItem>
